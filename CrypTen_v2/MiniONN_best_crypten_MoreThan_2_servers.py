...
class PerformGrater(MultiProcessTestCase):
    ...
    def test(self):
        ...
        if self.rank==0: # ALICE
            ...
            model = crypten.load('./SCML_Project/training/models/minionn/checkpoint_cpu_cpu.pt', dummy_model=resnet_model, src=0)
            ...
            model = crypten.load('./SCML_Project/training/models/minionn/minionn_random.pt', dummy_model=resnet_model, src=0)
            dummy_input = torch.empty((1, 3, 32, 32))
            private_model = crypten.nn.from_pytorch(model.double(), dummy_input.double())
            private_model.encrypt(src=0)
